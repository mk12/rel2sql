...
impl<'a> TryFrom<ulc::Term<'a>> for Formula<'a> {
    ...
    fn try_from(term: Term) -> Result<Formula, Error> {
        use self::Formula::{Conn, Exists, Pred, Rel};
        match term {
            ...
            Term::App(fun, args) => match fun.try_into() {
                Ok(Operator::Conn(conn)) => Ok(Conn(conn, from_vec(args)?)),
...
impl<'a> TryFrom<&'a str> for Operator<'a> {
    ...
    fn try_from(s: &str) -> Result<Operator, ()> {
        match s {
            "!" => Ok(Operator::Conn(Connective::Not)),
            "&" => Ok(Operator::Conn(Connective::And)),
            "|" => Ok(Operator::Conn(Connective::Or)),
            _ => {
                if EXPRESSION_OPERATORS.contains(&s) {
                    Ok(Operator::Arith(s))
                } else if FORMULA_OPERATORS.contains(&s) {
                    Ok(Operator::Comp(s))
                } else {
                    Err(())
                }
